...
const commonScriptsToExport = {
  start: {
    // dev: `npx kill-port ${process.env.PORT} && ts-node -r tsconfig-paths/register src/`,
    dev: `npx kill-port ${process.env.PORT}  && nodemon -e ts,tsx -i ./src/tests -i index.d.ts -x ts-node -r tsconfig-paths/register src/`,
    prod: `npx kill-port ${process.env.PORT} && node dist/main.js`,
  },
  build: {
    wp: {
      dev: series.nps("clean.dist", "webpack.dev"),
      prod: series.nps("clean.dist", "webpack.prod"),
    },
    dev: series.nps("clean.dist", "tsc"),
    prod: series.nps("clean.dist", "tsc"),
  },
  webpack: {
  ...
  node: {
    ...
    flame:
      "npx kill-port 5002 && nodemon -e ts,ts-node -r tsconfig-paths/register src/",
  },
  grpc: {
  ...
  apollo: {
    current: {
      ...
      --name ${process.env.SERVICE_NAME} --schema - \
      --routing-url http://${process.env.SERVICE_NAME}.svc.cluster.local:${process.env.PORT}/graphql`,
      introspect: `rover subgraph introspect http://${process.env.HOST}:${process.env.PORT}/graphql > ${process.env.SERVICE_NAME}-schema-rover.graphql`,
    },
    all: `rover subgraph introspect \
    http://localhost:${process.env.PORT}/graphql | \
    APOLLO_KEY=service:barfriends:HK36HSViS1rAb1l5wNG67w \
    rover subgraph publish barfriends@${process.env.NODE_ENV} \
    --name ${process.env.SERVICE_NAME} --schema - \
    --routing-url http://${process.env.SERVICE_NAME}.svc.cluster.local:${process.env.PORT}/graphql`,
    introspect: `rover subgraph introspect http://${process.env.HOST}:${process.env.PORT}/graphql > ${process.env.SERVICE_NAME}-schema-rover.graphql`,
  },
